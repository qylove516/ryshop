...
class AdminSite(object):
    ...
    def create_model_admin_view(self, admin_view_class, model, option_class):
        # self.get_view_class(admin_view_class, option_class)  返回的是一个类
    ...
    def get_urls(self):
        ...
        for model, admin_class in iteritems(self._registry):
            view_urls = [
                re_path(
                    _path,  # 路径
                    wrap(self.create_model_admin_view(clz, model, admin_class)),  # 函数 model, Goods, GoodsAdmin
                    name=name % (model._meta.app_label, model._meta.model_name)  # 反向映射
                )
            ...
            # print()
            # _path: ^$
            # clz: <class 'xadmin.views.list.ListAdminView'>
            # name: %s_%s_changelist
            # _path: ^add/$
            # clz: <class 'xadmin.views.edit.CreateAdminView'>
            # name: %s_%s_add
            # _path: ^(.+)/delete/$
            # clz: <class 'xadmin.views.delete.DeleteAdminView'>
            # name: %s_%s_delete
            # _path: ^(.+)/update/$
            # clz: <class 'xadmin.views.edit.UpdateAdminView'>
            # name: %s_%s_change
